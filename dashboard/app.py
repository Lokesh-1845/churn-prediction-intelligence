...
# ============================================================
# OVERVIEW BUSINESS IMPACT ASSUMPTIONS
# ============================================================

ESTIMATED_VALUE_PER_CRITICAL_CUSTOMER = 150000
ESTIMATED_RETENTION_RATE = 0.15
...
def generate_customer_names(customer_ids):
    ...
    for customer_id in customer_ids:
        ...
        names.append(
            f"{FIRST_NAMES[first_index]} {LAST_NAMES[last_index]}"
        )
...
st.markdown(
    ...
    .critical-risk-card .card-header {
        display: flex;
        align-items: center;
        gap: 9px;
        color: #9f1239;
    ...
    .critical-risk-card .card-description {
        color: #6b7280;
        font-size: 0.9rem;
        line-height: 1.4;
    }

    .critical-risk-card .card-footer {
        margin-top: 12px;
        color: #be123c;
        font-size: 0.78rem;
        font-weight: 700;
    ...
    .business-impact-card .card-header {
        display: flex;
        align-items: center;
        gap: 9px;
        color: #1d4ed8;
    ...
    .business-impact-card .card-value {
        color: #1e3a8a;
        font-size: 2.65rem;
        font-weight: 900;
        line-height: 1;
    ...
    .business-impact-card .card-description {
        color: #6b7280;
        font-size: 0.9rem;
        line-height: 1.4;
    }

    .business-impact-card .card-footer {
        margin-top: 12px;
        color: #1d4ed8;
        font-size: 0.78rem;
        font-weight: 700;
...
def normalize_shap_values(
    ...
    if values.ndim == 3:
        ...
        if (
            values.shape[0] == expected_rows
            and
            values.shape[1] == expected_features
        ):
...
def calculate_model_performance(
    ...
    top_n = max(
        1,
        int(len(evaluation) * 0.10)
    )

    top_10 = evaluation.head(top_n)

    actual_churners = (
        evaluation["actual"].sum()
    )
    ...
    return {
        ...
        "brier": brier,
        "confusion_matrix": cm,
        "recall_at_top10": recall_at_top10,
...
def business_impact_simulator(predictions):
    ...
    with c1:
        ...
        capacity = st.number_input(
            ...
            value=min(2000, high_risk_count),
            step=(
                100
    ...
    save_rate = save_rate_input / 100

    expected_saved = contacted * save_rate
    ...
    if net_impact >= 0:
        ...
        st.info(
            f"Revenue / Campaign Cost: "
            f"**{revenue_cost_ratio:,.2f}x**"
        )
...
if page == "🏠 Overview":
    ...
    with right:
        ...
        estimated_business_impact = (
            critical
            * ESTIMATED_VALUE_PER_CRITICAL_CUSTOMER
        ...
        with card2:
            ...
            st.markdown(
                ...
                <div class="business-impact-card">
                    ...
                    <div class="card-value">
                        ${estimated_business_impact:,.0f}
                    </div>

                    <div class="card-description">
                        Estimated recoverable customer
                        value from critical-risk customers.
                    </div>

                    <div class="card-footer">
                        Based on {ESTIMATED_RETENTION_RATE:.0%}
        ...
        st.caption(
            f"Business impact estimate assumes "
            f"${ESTIMATED_VALUE_PER_CRITICAL_CUSTOMER:,.0f} "
            f"recoverable value per critical customer and a "
            f"{ESTIMATED_RETENTION_RATE:.0%} retention success rate. "
            f"This is a business assumption, not a model prediction."
        )
    ...
    st.caption(
        "Customers ranked by highest predicted churn probability."
    )
...
# ============================================================
    ...
    selected_customer_id = (
        customer_labels[selected_label]
    )
    ...
    customer_name = (
        selected_prediction["customer_name"]
    )
